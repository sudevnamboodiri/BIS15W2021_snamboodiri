...
ui <- fluidPage(
    selectInput("fill", "Select fill type", choices = c("thermoregulation", "trophic.guild")),
  plotOutput("plot", width = "500px", height = "400px")
)

server <- function(input, output) {
  output$plot <- renderPlot({
    ggplot(homerange, aes_string(x = "locomotion", fill = input$fill)) + geom_bar(position = 'dodge') + theme_light(base_size = 18)
  })
