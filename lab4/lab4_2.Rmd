...
```{r}
bovines = select(mammals, family, genus, species, mass, gestation)
filter(bovines, family == 'Bovidae' & mass > 450000)
```
...
```{r}
monke <- filter(mammals, genus == 'Lophocebus' | genus == 'Macaca' | genus == 'Erythrocebus')

smallestbaby <- filter(monke, newborn == min(monke[,'newborn']))

cat('The primate with the smallest newborn baby is' , smallestbaby$species[1], 'of genus', smallestbaby$genus[1])
