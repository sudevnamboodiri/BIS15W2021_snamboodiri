...
homerange %>%
  ...
  summarise(count = n()) %>%
  ggplot(aes(x=thermoregulation, fill = thermoregulation, y = count)) + geom_col() +
  labs(title = "Number of Ectotherm vs Endotherms", x = "Therm", y = NULL)
...
```{r}
homerange %>%
  ggplot(aes(x=log10.mass, y = homerange)) + geom_point()
```
